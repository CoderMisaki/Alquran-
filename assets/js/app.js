...
function createJuzSurahCard(m, list) {
  if (!validSurahNumber(m.number)) return null;

  const b = createTextElement('button', 'surah-card tap-effect', '');
  b.type = 'button';

  b.addEventListener('click', () => {
    const src = m._src || (Array.isArray(list) ? list : []).find((x) => (x.meta?.number || x.number) === m.number);
    if (src?.ayahsAr) {
...
function renderJuzSurahList(list) {
  ...
  const mapped = mapJuzSurahData(list);
  const frag = document.createDocumentFragment();

  mapped.forEach((m) => {
    const card = createJuzSurahCard(m, list);
